...
import { Lock, Loader2, Building2, User } from "lucide-react";
...
export default function LoginPage() {
  ...
  const doLogin = async (uname: string, pwd: string) => {
    ...
    try {
      const r = (await apiClient.post("/auth/login", { username: uname, password: pwd })) as unknown as { access_token: string };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-muted/20 p-4">
      <Card className="w-full max-w-sm">
        ...
        <CardContent>
          <form onSubmit={(e) => { e.preventDefault(); if (username && password) doLogin(username, password); }} className="space-y-3">
            <div className="relative">
              <User className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input value={username} onChange={(e) => setUsername(e.target.value)} placeholder="Username" className="pl-9" autoFocus />
            </div>
            <div className="relative">
              <Lock className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
              <Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} placeholder="Password" className="pl-9" />
            </div>
